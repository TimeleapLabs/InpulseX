<script>
	...
	import tokenAbi from '@openzeppelin/contracts/build/contracts/IERC1155.json';
	...
	const getStakeStats = async () => {
		...
		if (stake.gt(0)) {
			...
			const stakePool = await token.balanceOf(address, nftId);
	...
	const onProvider = async () => {
		...
		rewardToken = new ethers.Contract(rewardTokenAddress, rewardTokenAbi.abi, signer);
	...
	const onStake = async () => {
		if (amount.toString() === '0') {
			return toast.error('Must stake more than 0 NFTs!');
		}
		busy = true;
		await toast
			.promise(stake(), {
				...
				error: 'There was an issue staking your NFTs!'
			})
			.catch(() => null);
		busy = false;
		getStakeStats();
	...
	const onUnstake = async () => {
		...
		await toast
			.promise(unstake(), {
				...
				error: 'There was an issue unstaking your NFTs!'
			})
			.catch(() => null);
		busy = false;
		getStakeStats();
